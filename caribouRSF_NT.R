...
doEvent.caribouRSF_NT = function(sim, eventTime, eventType) {
  switch(
    ...
    init = {
      ...
      sim <- scheduleEvent(sim, P(sim)$.plotInitialTime, "caribouRSF_NT", "plot")
      sim <- scheduleEvent(sim, P(sim)$.saveInitialTime, "caribouRSF_NT", "save")
    },
    plot = {
      # ! ----- EDIT BELOW ----- ! #
      # do stuff for this event
      ...
      # e.g.,
      #sim <- scheduleEvent(sim, time(sim) + P(sim)$.plotInterval, "caribouRSF_NT", "plot")
    ...
    save = {
      ...
      # e.g.,
      # sim <- scheduleEvent(sim, time(sim) + P(sim)$.saveInterval, "caribouRSF_NT", "save")
    ...
    event1 = {
      ...
      # e.g.,
      # sim <- scheduleEvent(sim, time(sim) + increment, "caribouRSF_NT", "templateEvent")
